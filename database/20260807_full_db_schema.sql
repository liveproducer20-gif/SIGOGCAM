...
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name=N'UX_distribucion_encargado_circuito' AND object_id=OBJECT_ID(N'dbo.distribucion_encargados'))
    CREATE UNIQUE INDEX UX_distribucion_encargado_circuito ON dbo.distribucion_encargados(distribucion_id,circuito_id) WHERE tipo_responsabilidad=N'ENCARGADO_CIRCUITO' AND deleted_at IS NULL;
GO
