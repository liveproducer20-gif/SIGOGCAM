...
CREATE TABLE dbo.alertas_soporte (
    ...
    descripcion         NVARCHAR(MAX) NULL,
    estado              NVARCHAR(20) NOT NULL CONSTRAINT DF_alertas_soporte_estado DEFAULT ('ABIERTA'),
    fecha_creacion      DATETIME2 NOT NULL CONSTRAINT DF_alertas_soporte_fecha DEFAULT (SYSDATETIME()),
    fecha_cierre        DATETIME2 NULL,
    asignado_por_id     INT NULL,
    creado_por_id       INT NULL,
    CONSTRAINT PK_alertas_soporte PRIMARY KEY (id),
    CONSTRAINT UQ_alertas_soporte_codigo UNIQUE (codigo)
);
GO
...
CREATE TABLE dbo.alertas_soporte_comentarios (
    ...
    fecha_creacion  DATETIME2 NOT NULL DEFAULT (SYSDATETIME()),
    CONSTRAINT PK_alertas_soporte_comentarios PRIMARY KEY (id),
    CONSTRAINT FK_alertas_soporte_comentarios_alerta FOREIGN KEY (alerta_id) REFERENCES dbo.alertas_soporte(id) ON DELETE CASCADE
);
GO
...
CREATE TABLE dbo.alertas_soporte_historial (
    ...
    CONSTRAINT PK_alertas_soporte_historial PRIMARY KEY (id),
    CONSTRAINT FK_alertas_soporte_historial_alerta FOREIGN KEY (alerta_id) REFERENCES dbo.alertas_soporte(id) ON DELETE CASCADE
);
GO
