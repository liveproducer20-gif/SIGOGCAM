...
<section class="dashboard admin-workspace">
    ...
    <header class="admin-section-heading"><div><h2>Rutas operativas</h2><p>Distrito, turno y franja horaria relacionada.</p></div><div class="admin-route-actions"><span><?= count($adminData['rutas']) ?> rutas</span><a class="admin-import-button admin-template-button" href="/admin/rutas/plantilla.csv">Descargar plantilla CSV</a><?php if ($can('catalogos.crear')): ?><form method="post" action="/admin/rutas/importar" enctype="multipart/form-data"><input type="hidden" name="import_action" value="preview"><label class="admin-import-button" for="route-csv">Importar CSV</label><input id="route-csv" name="archivo_csv" type="file" accept=".csv,text/csv" hidden required onchange="this.form.submit()"></form><?php endif; ?></div></header>
    ...
    <section class="admin-list-filters" aria-label="Filtros de rutas">
        <label>Distrito<select data-table-filter="admin-routes-table" data-filter-attribute="district"><option value="">Todos los distritos</option><?php foreach (($refs['distritos'] ?? []) as $district): ?><option value="<?= (int)$district['id'] ?>"><?= $e($district['nombre']) ?></option><?php endforeach; ?></select></label>
        <label>Circuito<select data-table-filter="admin-routes-table" data-filter-attribute="circuit"><option value="">Todos los circuitos</option><option value="__empty__">Sin circuito</option><?php foreach ($routeCircuits as $circuitId=>$circuitName): ?><option value="<?= (int)$circuitId ?>"><?= $e($circuitName) ?></option><?php endforeach; ?></select></label>
        <button type="button" class="secondary" data-clear-table-filters="admin-routes-table">Limpiar filtros</button>
    </section>
    <section class="table-wrap"><table id="admin-routes-table"><thead><tr><th>Ruta</th><th>Distrito</th><th>Circuito</th><th>Turno</th><th>Horario</th><th>Encargado</th><th>Estado</th><th>Acciones</th></tr></thead><tbody><?php foreach ($adminData['rutas'] as $item): $payload=['id'=>$item['id'],'nombre'=>$item['nombre'],'distrito_id'=>$item['distrito_id'],'turno_id'=>$item['turno_id'],'hora_inicio'=>$item['hora_inicio'],'hora_fin'=>$item['hora_fin'],'asignar_encargado'=>(bool)$item['asignar_encargado'],'activo'=>(bool)$item['activo']]; ?><tr data-district="<?= (int)($item['distrito_id'] ?? 0) ?>" data-circuit="<?= !empty($item['circuito_id']) ? (int)$item['circuito_id'] : '' ?>"><td><strong><?= $e($item['nombre']) ?></strong></td><td><?= $e($item['distrito'] ?? '—') ?></td><td><?= $e($item['circuito'] ?? 'Sin circuito') ?></td><td><?= $e($item['turno'] ?? '—') ?></td><td><?= $e(($item['hora_inicio'] ?? '—').' – '.($item['hora_fin'] ?? '—')) ?></td><td><span class="status-pill <?= $item['asignar_encargado'] ? 'is-active' : '' ?>"><?= $item['asignar_encargado'] ? 'Permitido' : 'No requerido' ?></span></td><td><span class="status-pill <?= $item['activo'] ? 'is-active' : '' ?>"><?= $item['activo'] ? 'Activa' : 'Inactiva' ?></span></td><td class="actions"><?php if ($can('catalogos.editar')): ?><button type="button" class="secondary" data-edit-target="#form-ruta" data-payload="<?= $json($payload) ?>">Editar</button><?php endif; ?><?php if ($can('catalogos.estado')): ?><form method="post" action="/admin/eliminar" class="inline-form"><input type="hidden" name="entity" value="ruta"><input type="hidden" name="tab" value="rutas"><input type="hidden" name="id" value="<?= (int)$item['id'] ?>"><button class="danger">Eliminar</button></form><?php endif; ?></td></tr><?php endforeach; ?></tbody></table></section>
    <?php endif; ?>
    ...
    <header class="admin-section-heading circuit-heading">
        ...
        <?php if ($can('circuitos.crear')): ?><button type="button" data-circuit-create>+ Crear circuito</button><?php endif; ?>
    </header>
    ...
    <form class="form-panel admin-form circuit-form" method="post" action="/admin" id="form-circuito" hidden>
        <input type="hidden" name="entity" value="circuito"><input type="hidden" name="tab" value="circuitos"><input type="hidden" name="id">
        <div class="admin-form-title"><h3 data-circuit-form-title>Nuevo circuito</h3><button type="reset" class="secondary" data-circuit-cancel>Cancelar</button></div>
        <div class="form-grid">
            ...
            <label class="span-3">Consignas<textarea name="consignas" rows="3"></textarea></label>
            <label class="span-3">Observaciones<textarea name="observaciones" rows="3"></textarea></label>
            <label class="span-3">Perímetro escrito<textarea name="perimetro" rows="4" placeholder="Describa calles, límites y referencias del perímetro"></textarea></label>
        </div><button type="submit">Guardar circuito</button>
    </form>
    <?php endif; ?>
    ...
    <dialog class="admin-route-import-dialog admin-service-place-import-dialog" id="servicePlaceImportDialog" aria-labelledby="service-place-csv-title">
        <form method="post" action="/admin/lugares-servicio/importar">
            ...
            <header><div><span class="eyebrow">Validación previa</span><h3 id="service-place-csv-title">Importar lugares desde CSV</h3><p>Revise cada fila. Al confirmar se guardarán únicamente los registros válidos.</p></div><button type="button" aria-label="Cerrar modal" data-import-close>&times;</button></header>
            <div class="admin-route-import-summary"><span>Total: <b><?= (int)($importPreview['total'] ?? 0) ?></b></span><span class="is-valid">Válidos: <b><?= (int)($importPreview['validos'] ?? 0) ?></b></span><span class="is-warning">Duplicados: <b><?= (int)($importPreview['duplicados'] ?? 0) ?></b></span><span class="is-invalid">Con errores: <b><?= (int)($importPreview['rechazados'] ?? 0) ?></b></span><span>Rutas encontradas: <b><?= (int)($importPreview['rutas_encontradas'] ?? 0) ?></b></span><span class="is-invalid">Sin ruta: <b><?= (int)($importPreview['rutas_no_encontradas'] ?? 0) ?></b></span></div>
            <div class="admin-route-import-table"><table><thead><tr><th>Estado</th><th>Ruta</th><th>Lugar de servicio</th><th>Horario</th><th>Lugar de formación</th><th>Acción</th><th>Detalles</th></tr></thead><tbody>
            <?php foreach (($importPreview['filas'] ?? []) as $idx => $row): $isValid=!empty($row['valida']);$isDup=!empty($row['duplicado']);$rowClass=$isValid&&!$isDup?'route-import-valida':($isDup?'route-import-existente':'route-import-error');$detailId='csv-detail-'.$idx; ?>
                <tr class="<?= $rowClass ?>"><td><?php if ($isValid && !$isDup): ?><span class="route-import-status">✓ Válido</span><?php elseif ($isDup): ?><span class="route-import-status">⚠ Duplicado</span><?php else: ?><span class="route-import-status">✕ Error</span><?php endif; ?><?php if (!empty($row['errores'])): ?><ul class="csv-errors"><?php foreach ($row['errores'] as $reason): ?><li><?= $e($reason) ?></li><?php endforeach; ?></ul><?php endif; ?></td><td><?= $e($row['ruta'] ?? '') ?></td><td><strong><?= $e($row['lugar_servicio'] ?? '') ?></strong></td><td><?= $e($row['horario'] ?: '...') ?></td><td><?= $e($row['lugar_formacion'] ?: '...') ?></td><td><?php if (($row['tipo_duplicado'] ?? '') === 'BASE_DATOS'): ?><select name="existing_action[<?= (int)$row['fila'] ?>]"><option value="OMITIR">Omitir</option><option value="ACTUALIZAR">Actualizar existente</option></select><?php elseif (($row['tipo_duplicado'] ?? '') === 'ARCHIVO'): ?><select disabled><option>Omitir</option></select><?php else: ?>—<?php endif; ?></td><td><button type="button" class="csv-toggle-detail" data-target="<?= $detailId ?>" aria-expanded="false" aria-controls="<?= $detailId ?>">▼</button></td></tr>
