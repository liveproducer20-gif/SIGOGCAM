...
<section class="dashboard admin-workspace">
    ...
    <section class="table-wrap"><table><thead><tr><th>Ruta</th><th>Distrito</th><th>Turno</th><th>Horario</th><th>Estado</th><th>Acciones</th></tr></thead><tbody><?php foreach ($adminData['rutas'] as $item): $payload=['id'=>$item['id'],'nombre'=>$item['nombre'],'distrito_id'=>$item['distrito_id'],'turno_id'=>$item['turno_id'],'hora_inicio'=>$item['hora_inicio'],'hora_fin'=>$item['hora_fin'],'activo'=>(bool)$item['activo']]; ?><tr><td><strong><?= $e($item['nombre']) ?></strong></td><td><?= $e($item['distrito'] ?? '—') ?></td><td><?= $e($item['turno'] ?? '—') ?></td><td><?= $e(($item['hora_inicio'] ?? '—').' – '.($item['hora_fin'] ?? '—')) ?></td><td><span class="status-pill <?= $item['activo'] ? 'is-active' : '' ?>"><?= $item['activo'] ? 'Activa' : 'Inactiva' ?></span></td><td class="actions"><?php if ($can('catalogos.editar')): ?><button type="button" class="secondary" data-edit-target="#form-ruta" data-payload="<?= $json($payload) ?>">Editar</button><?php endif; ?><?php if ($can('catalogos.estado')): ?><form method="post" action="/admin/eliminar" class="inline-form"><input type="hidden" name="entity" value="ruta"><input type="hidden" name="tab" value="rutas"><input type="hidden" name="id" value="<?= (int)$item['id'] ?>"><button class="danger">Eliminar</button></form><?php endif; ?></td></tr><?php endforeach; ?></tbody></table></section>
    <?php endif; ?>
