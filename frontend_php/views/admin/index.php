...
<section class="dashboard admin-workspace">
    ...
    <header class="admin-section-heading circuit-heading">
        ...
        <div class="admin-route-actions"><a class="admin-import-button admin-template-button" href="/admin/rutas/plantilla.csv">Descargar plantilla CSV</a><?php if ($can('circuitos.rutas')): ?><button type="button" class="admin-import-button" data-circuit-import-open>Importar CSV</button><?php endif; ?><?php if ($can('circuitos.crear')): ?><button type="button" data-circuit-create>+ Crear circuito</button><?php endif; ?></div>
    </header>
    ...
    <form class="form-panel admin-form" method="post" action="/admin" id="form-lugar">
        ...
        <div class="form-grid">
            ...
            <label>Horario<input type="text" name="ubicacion_especifica" placeholder="Ej. 09:30 a 17:00"></label>
        </div>
        <div id="lugares-list" class="form-grid" style="margin-top:1rem">
            <label class="span-2 lugar-row">Nombre del lugar de servicio
        ...
        <button type="submit" data-place-submit>Guardar lugares</button>
    </form>
    <script>
    (function(){
    ...
    <section class="admin-list-filters has-three-filters" aria-label="Filtros de lugares de servicio">
        ...
        <label>Circuito<select data-table-filter="admin-places-table" data-filter-attribute="circuit"><option value="">Todos los circuitos</option><option value="__empty__">Sin circuito</option><?php foreach ($placeCircuits as $circuitId=>$circuitName): ?><option value="<?= (int)$circuitId ?>"><?= $e($circuitName) ?></option><?php endforeach; ?></select></label>
