...
class _AnnImage extends StatelessWidget {
  final AnnMdl ann;
  const _AnnImage({required this.ann});
  @override
        ...
        children: [
          Center(
            child: InteractiveViewer(
              child: ann.imgUrl != null
                  ? Image.network(ann.imgUrl!, fit: BoxFit.contain)
    ...
    child: Column(
      ...
      children: [
        Wrap(
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                color: _accent(ann.prioridad).withValues(alpha: .11),
                borderRadius: BorderRadius.circular(20),
              ),
              child: Text(
                ann.prioridad,
                style: TextStyle(
                  color: _accent(ann.prioridad),
                  fontSize: 10.5,
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: ann.publicado
                    ? AdmTokens.success.withValues(alpha: .1)
                    : AdmTokens.grey100,
                borderRadius: BorderRadius.circular(20),
              ),
              child: Text(
                ann.publicado ? 'Publicado' : 'Oculto',
                style: TextStyle(
                  color: ann.publicado ? AdmTokens.success : AdmTokens.grey500,
                  fontSize: 10.5,
                  fontWeight: FontWeight.w700,
                ),
              ),
            ...
            Text(
              '${_two(ann.fecPub.day)}/${_two(ann.fecPub.month)}/${ann.fecPub.year} · ${_two(ann.fecPub.hour)}:${_two(ann.fecPub.minute)}',
            ...
            if (canManage)
              PopupMenuButton<String>(
                ...
                itemBuilder: (_) => [
                  ...
                  PopupMenuItem(
                    value: 'toggle',
                    child: Text(ann.publicado ? 'Ocultar' : 'Publicar'),
                  ),
        ...
        Text(
          ...
          maxLines: 3,
          overflow: TextOverflow.ellipsis,
          style: AdmTokens.body.copyWith(height: 1.4),
        ),
        const SizedBox(height: 10),
        TextButton.icon(
          onPressed: () => _details(context),
          icon: const Icon(Icons.arrow_forward_rounded, size: 17),
          label: Text(canManage ? 'Ver anuncio' : 'Leer más'),
  ...
  Future<void> _details(BuildContext context) => showDialog<void>(
    ...
    builder: (_) => AlertDialog(
      ...
      content: ResponsiveDialogBody(
        child: Text(ann.desc, style: AdmTokens.body.copyWith(height: 1.5)),
      ),
      actions: [
        FilledButton(
          onPressed: () => Navigator.pop(context),
...
Color _accent(String value) {
  ...
  if (v.contains('important')) return AdmTokens.warning;
  return AdmTokens.secondary;
}
