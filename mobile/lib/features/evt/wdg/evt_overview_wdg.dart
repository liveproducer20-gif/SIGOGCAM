            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    '$value',
                    style: AdmTokens.statValue.copyWith(fontSize: 25),
                  ),
                ],
              ),
            ),
...
class _GroupHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(
    children: [
      ...
      Text(
        name,
        style: AdmTokens.label.copyWith(
      ...
      Container(
        ...
        child: Text(
          ...
          style: AdmTokens.bodySmall.copyWith(color: AdmTokens.primary),
        ),
      ),
    ],
  );
...
class _ModernEventCardState extends State<_ModernEventCard> {
  ...
  Widget build(BuildContext context) {
    ...
    final progress = e.convocados == 0
        ? 0.0
        : (e.confirmados / e.convocados).clamp(0.0, 1.0);
    return MouseRegion(
      onEnter: (_) => setState(() => hovered = true),
      onExit: (_) => setState(() => hovered = false),
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: hovered
                ? AdmTokens.secondary.withValues(alpha: .35)
                : AdmTokens.grey100,
          ),
          boxShadow: hovered ? AdmTokens.hoverShadow : AdmTokens.cardShadow,
        ),
            ...
            return compact
                ...
                : IntrinsicHeight(
                    child: Row(
                      crossAxisAlignment: CrossAxisAlignment.stretch,
                      children: [
                        SizedBox(width: c.maxWidth * .34, child: image),
                        Expanded(child: body),
                      ],
                    ),
                  );
  ...
  Widget build(BuildContext context) => Stack(
    ...
    children: [
      ...
      Positioned(
        ...
        left: 11,
        child: _Pill(
          label: EvtEstadoStyle.label(evt.estado),
    ...
    child: Column(
      ...
      children: [
          ...
          children: [
            Expanded(
              child: Wrap(
                ...
                runSpacing: 6,
                children: [
                  _Pill(label: evt.tipo, color: AdmTokens.secondary),
                  _Pill(label: evt.prioridad, color: _priority(evt.prioridad)),
                ],
              ),
            ),
            if (canManage)
              PopupMenuButton<String>(
                tooltip: 'Acciones',
                onSelected: (v) {
                  if (v == 'edit') onEditar?.call(evt);
                  if (v == 'delete') onEliminar?.call(evt);
                  if (v.startsWith('state:')) {
                    onEstado?.call(evt, v.substring(6));
                  }
                },
                itemBuilder: (_) => const [
                  PopupMenuItem(value: 'edit', child: Text('Editar')),
                  PopupMenuItem(
                    value: 'state:PLANIFICADO',
                    child: Text('Marcar planificado'),
                  ),
                  PopupMenuItem(
                    value: 'state:EN_CURSO',
                    child: Text('Marcar en curso'),
                  ),
                  PopupMenuItem(
        ...
        Text(
          ...
          style: AdmTokens.h2.copyWith(color: AdmTokens.primary, fontSize: 18),
        ),
        if (evt.descripcion.isNotEmpty) ...[
          ...
          Text(
            ...
            maxLines: 2,
            overflow: TextOverflow.ellipsis,
            style: AdmTokens.bodySmall.copyWith(height: 1.35),
          ),
        ],
        const SizedBox(height: 10),
        Wrap(
          ...
          runSpacing: 7,
          children: [
            _Meta(Icons.calendar_month_outlined, evt.fecha),
            _Meta(
              Icons.schedule_outlined,
              evt.hora.isEmpty ? 'Sin hora' : evt.hora,
            ),
            if (evt.lugar.isNotEmpty)
              _Meta(Icons.location_on_outlined, evt.lugar),
          ],
        ),
        const SizedBox(height: 12),
        Row(
          children: [
            _MiniStat('Convocados', evt.convocados, AdmTokens.secondary),
            _MiniStat('Confirmados', evt.confirmados, AdmTokens.success),
            _MiniStat('Pendientes', pending, AdmTokens.warning),
          ],
        ),
        ...
        Wrap(
          ...
          runSpacing: 6,
          children: [
            if (evt.pdfUrl != null || evt.pdfNombre != null)
              OutlinedButton.icon(
                onPressed: () => _showPdf(context, evt),
                icon: const Icon(Icons.description_outlined, size: 17),
                label: Text(evt.pdfNombre ?? 'Archivo'),
              ),
            if (evt.lugar.isNotEmpty)
              TextButton.icon(
                onPressed: () => openExternalUrl(_mapsUrl(evt.lugar)),
                icon: const Icon(Icons.location_on_outlined, size: 17),
                label: const Text('Abrir ubicación'),
              ),
            if (!canManage)
              TextButton.icon(
                onPressed: () => _showDetails(context, evt),
      ...
      children: [
        ...
        Flexible(
          child: Text(
            '$value $label',
            overflow: TextOverflow.ellipsis,
            style: AdmTokens.bodySmall.copyWith(
...
class _EventAside extends StatelessWidget {
  final List<EvtMdl> items;
  const _EventAside({required this.items});
  @override
  Widget build(BuildContext context) {
    final now = DateTime.now();
    ...
    return Column(
      children: [
        _AsideBox(
          ...
          child: _MiniCalendar(now: now, items: items),
        ),
        ...
        _AsideBox(
          ...
          child: Column(
            children: [
              for (final e in types.entries.take(5))
                Padding(
                  padding: const EdgeInsets.only(bottom: 9),
                  child: Row(
                    children: [
                      Expanded(child: Text(e.key, style: AdmTokens.bodySmall)),
                      Text('${e.value}', style: AdmTokens.label),
                    ],
                  ),
                ),
            ],
                        ...
                        child: Column(
                          ...
                          children: [
                            Text(
                              ...
                              maxLines: 1,
                              overflow: TextOverflow.ellipsis,
                              style: AdmTokens.label,
                            ),
                            Text(e.fecha, style: AdmTokens.bodySmall),
                          ],
                        ),
                      ),
                    ],
...
class _AsideBox extends StatelessWidget {
  final String title;
  final IconData icon;
  final Widget child;
  const _AsideBox({
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(
      ...
      children: [
        Row(
          children: [
            Icon(icon, size: 19, color: AdmTokens.primary),
            const SizedBox(width: 8),
            Text(
              title,
              style: AdmTokens.label.copyWith(color: AdmTokens.primary),
            ),
          ],
        ),
        const SizedBox(height: 14),
...
class _MiniCalendar extends StatelessWidget {
  final DateTime now;
  final List<EvtMdl> items;
  const _MiniCalendar({required this.now, required this.items});
  @override
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        Text('${_month(now.month)} ${now.year}', style: AdmTokens.label),
        const SizedBox(height: 10),
        GridView.builder(
          shrinkWrap: true,
              ...
              child: Text(
                ...
                style: TextStyle(
                  ...
                  color: today
                      ...
                      ? AdmTokens.primary
                      : AdmTokens.grey600,
                  fontWeight: has || today ? FontWeight.w700 : FontWeight.w400,
                ),
              ),
            );
          },
...
class _EmptyEvents extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: const Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        Icon(Icons.event_busy_outlined, size: 48, color: AdmTokens.grey400),
        SizedBox(height: 10),
        Text(
...
String _group(EvtMdl e) {
  final d = _date(e);
  if (d == null) return 'PRÓXIMAMENTE';
  final now = DateTime.now();
...
Color _priority(String value) {
  final v = value.toLowerCase();
  if (v.contains('urgent')) return AdmTokens.error;
  if (v.contains('important') || v.contains('alta')) return AdmTokens.warning;
  return AdmTokens.grey500;
}
...
Future<void> _showPdf(BuildContext context, EvtMdl evt) => showDialog<void>(
  context: context,
  builder: (_) => AlertDialog(
    title: Text(evt.pdfNombre ?? 'Archivo adjunto'),
...
Future<void> _showDetails(BuildContext context, EvtMdl evt) => showDialog<void>(
  context: context,
  builder: (_) => AlertDialog(
    title: Text(evt.nom),
    content: SizedBox(
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            Text(
              evt.descripcion.isEmpty ? 'Sin descripción.' : evt.descripcion,
            ),
            const SizedBox(height: 16),
            _Meta(Icons.calendar_month_outlined, evt.fecha),
            const SizedBox(height: 8),
            _Meta(Icons.schedule_outlined, evt.hora),
            if (evt.lugar.isNotEmpty) ...[
              const SizedBox(height: 8),
              _Meta(Icons.location_on_outlined, evt.lugar),
            ],
