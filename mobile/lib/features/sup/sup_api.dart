...
class SupportApi {
  ...
  Future<SupportPage> list({
    ...
    final raw = await _client.getFull(
      Uri(path: 'soporte', queryParameters: query).toString(),
    );
    return SupportPage(
      tickets: (raw['datos'] as List<dynamic>? ?? [])
          .whereType<Map>()
  ...
  Future<SupportStats> stats() async => (await _client.get(
    'soporte/estadisticas',
    (v) => SupportStats.fromJson(Map<String, dynamic>.from(v as Map)),
  )).datos!;
  Future<SupportDetail> detail(int id) async => (await _client.get(
    'soporte/$id',
    (v) => SupportDetail.fromJson(Map<String, dynamic>.from(v as Map)),
  )).datos!;
  ...
  Future<void> create({
    ...
    if (image?.dataUrl != null) {
      ...
      final uploaded = await _client.postBytes(
        'soporte/imagenes',
        bytes,
  ...
  Future<void> update(
    ...
    if (assign) {
      body['asignadoA'] = assignedTo;
      body['asignadoNombre'] = assignedName;
    }
    await _client.put('soporte/$id', body, (_) => true);
  }

  Future<void> comment(int id, String text, {bool internal = false}) async =>
      _client.post('soporte/$id/comentarios', {
        'comentario': text,
        'esInterno': internal,
      }, (_) => true);
