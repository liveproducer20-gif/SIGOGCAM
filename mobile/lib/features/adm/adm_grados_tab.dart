...
class GradosTab extends StatefulWidget {
  final AdmApi api;
  final int tabIndex;
  const GradosTab({super.key, required this.api, this.tabIndex = 0});

  @override
  State<GradosTab> createState() => _GradosTabState();
}

class _GradosTabState extends State<GradosTab> with AdmLazyTabMixin<GradosTab> {
  ...
  void initState() {
    super.initState();
    future = Future.value(const _GradeData(grades: [], personnel: []));
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<_GradeData>(
      ...
      builder: (context, snapshot) {
        final data =
            snapshot.data ?? const _GradeData(grades: [], personnel: []);
        final ordered = [...data.grades]
          ..sort((a, b) => admId(a).compareTo(admId(b)));
        final filtered = ordered.where((grade) {
          ...
          if (selectedGradeId != null && admId(grade) != selectedGradeId) {
            return false;
          }
          return true;
        }).toList();
        ...
        if (sort == 'Más personal') {
          filtered.sort((a, b) => data.countFor(b).compareTo(data.countFor(a)));
        }
        final active = ordered.where(admIsActive).length;
        final top = ordered.isEmpty ? null : ordered.last;
        final mostAssigned = ordered.isEmpty
            ? null
            : ([...ordered]..sort(
                    (a, b) => data.countFor(b).compareTo(data.countFor(a)),
                  ))
                  .first;
          ...
          child: Column(
            ...
            children: [
                ...
                children: [
                  const Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('Grados', style: AdmTokens.h1),
                        SizedBox(height: 5),
                        Text(
                          'Administración de grados jerárquicos y académicos del personal institucional.',
              ...
              AdminSummaryRow(
                cards: [
                  ...
                  AdminSummaryCardData(
                    ...
                    value: top?['nombre']?.toString() ?? 'Sin grados',
              ...
              if (snapshot.connectionState == ConnectionState.waiting)
                ...
                LayoutBuilder(
                  builder: (context, constraints) {
                    final cards = _GradeCards(
                      grades: filtered,
                      allGrades: ordered,
                      data: data,
                      onView: (g) => _showGrade(g, data),
                      onPersonnel: (g) => _showPersonnel(g, data),
                      onEdit: _edit,
                      onToggle: _toggle,
                      onDelete: _confirmDelete,
                    );
                    final hierarchy = _HierarchyPanel(
                      grades: ordered.reversed.toList(),
                      data: data,
                      selectedId: selectedGradeId,
                      onSelect: (id) => setState(
                        () =>
                            selectedGradeId = selectedGradeId == id ? null : id,
                      ),
                    );
              ...
              _GradeBottomStats(
                ...
                mostAssigned:
                    mostAssigned?['nombre']?.toString() ?? 'Sin datos',
                mostCount: mostAssigned == null
                    ? 0
                    : data.countFor(mostAssigned),
              ),
            ],
          ),
        );
  ...
  Future<void> _export(_GradeData data) async {
    ...
    for (var i = 0; i < ordered.length; i++) {
      ...
      rows.add(
        'GRD-${admId(grade).toString().padLeft(3, '0')},"${grade['nombre']}",${admIsActive(grade) ? 'Activo' : 'Inactivo'},${data.countFor(grade)},${i + 1}',
      );
    }
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text('Grados exportados en $path')));
  }

  Future<void> _showGrade(Map<String, dynamic> grade, _GradeData data) =>
      _showPersonnel(grade, data);

  Future<void> _showPersonnel(
    ...
    builder: (_) {
      ...
      return AlertDialog(
        title: Text('Personal — ${grade['nombre']}'),
        content: SizedBox(
          width: 560,
          child: personnel.isEmpty
              ? const Text('No existen funcionarios asignados a este grado.')
              : ListView(
                  shrinkWrap: true,
                  children: [
                    for (final person in personnel)
                      ListTile(
                        ...
                        subtitle: Text(
                          person['correo_institucional']?.toString() ??
                              'Sin correo',
                        ),
                      ),
                  ],
    ...
    child: LayoutBuilder(
      builder: (_, constraints) {
        ...
        if (constraints.maxWidth < 760) {
          return Column(
            children: [
              AdminSearchBar(onChanged: onSearch, hintText: 'Buscar grado...'),
  ...
  Widget build(BuildContext context) {
    if (grades.isEmpty) {
      return const SizedBox(
        height: 220,
        child: Center(child: Text('Sin grados para mostrar')),
      );
    }
    return Column(
      children: [
        for (final grade in grades)
          Padding(
            ...
            child: _GradeCard(
              grade: grade,
              level: allGrades.indexWhere((g) => admId(g) == admId(grade)) + 1,
              maxLevel: allGrades.length,
              assigned: data.countFor(grade),
              onView: () => onView(grade),
              onPersonnel: () => onPersonnel(grade),
              onEdit: () => onEdit(grade),
              onToggle: () => onToggle(grade),
              onDelete: () => onDelete(grade),
            ),
          ),
      ],
    );
...
class _GradeCardState extends State<_GradeCard> {
          ...
          builder: (_, c) {
            final main = [
              Container(
                ...
                child: Icon(visual.$1, color: Colors.white, size: 21),
              ),
              const SizedBox(width: 12),
              Container(
                width: 40,
                ...
                child: Text(
                  widget.level.toString().padLeft(2, '0'),
                  style: const TextStyle(
                    fontWeight: FontWeight.w800,
                    color: AdmTokens.grey700,
                  ),
                ),
              ),
              const SizedBox(width: 12),
              Expanded(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      _description(name, widget.level),
                      maxLines: 2,
                      overflow: TextOverflow.ellipsis,
                      style: const TextStyle(
                        fontSize: 11,
              ...
              const SizedBox(width: 12),
              AdmStateChip(active: admIsActive(widget.grade)),
              const SizedBox(width: 18),
              InkWell(
                onTap: widget.onPersonnel,
                  ...
                  PopupMenuButton<String>(
                    ...
                    itemBuilder: (_) => [
                      PopupMenuItem(
                        value: 'toggle',
                        child: Text(
                          admIsActive(widget.grade) ? 'Desactivar' : 'Activar',
                        ),
                      ),
                      const PopupMenuItem(
                        value: 'delete',
            ...
            return Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Row(children: main.take(4).toList()),
                const SizedBox(height: 12),
                Row(children: main.skip(4).toList()),
              ],
            );
          },
        ),
  ...
  static String _description(String name, int level) =>
      name.toLowerCase().contains('jefe')
      ? 'Coordinación y gestión institucional de nivel superior.'
      : 'Personal institucional correspondiente al nivel jerárquico $level.';
}

(IconData, Color) _gradeVisual(int level, int max) {
  if (level == max) {
    return (Icons.account_balance_outlined, const Color(0xFFD4A017));
  }
  const icons = [
      ...
      for (var i = 1; i <= maxLevel; i++)
        Expanded(
          child: Container(
            ...
            decoration: BoxDecoration(
              color: i <= level ? color : const Color(0xFFE5EAF0),
  ...
  Widget build(BuildContext context) => Tooltip(
    ...
    child: InkWell(
      ...
      child: Container(
        ...
        child: Icon(icon, size: 17, color: AdmTokens.primary),
      ),
    ),
  );
}
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(
      ...
      children: [
        ...
        const Text(
          'Selecciona un nivel para filtrar',
          style: TextStyle(fontSize: 11, color: AdmTokens.grey500),
        ),
        const SizedBox(height: 16),
        for (var i = 0; i < grades.length; i++)
          _HierarchyLevel(
            grade: grades[i],
            data: data,
            widthFactor: 1 - (i * .055).clamp(0, .38),
            selected: selectedId == admId(grades[i]),
            top: i == 0,
            onTap: () => onSelect(admId(grades[i])),
          ),
      ],
    ),
  );
          ...
          child: InkWell(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(10),
                border: Border.all(
                  color: color.withValues(alpha: selected ? 1 : .18),
                ),
              ),
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      grade['nombre']?.toString() ?? 'Grado',
                      maxLines: 1,
                      overflow: TextOverflow.ellipsis,
                      style: TextStyle(
                        fontSize: 11,
                        fontWeight: FontWeight.w700,
                        color: selected ? Colors.white : AdmTokens.grey800,
                      ),
                    ),
                  ),
                  Text(
...
class _GradeBottomStats extends StatelessWidget {
  final int total, active, personnel, mostCount;
  final String mostAssigned;
  const _GradeBottomStats({
    required this.total,
    required this.active,
    required this.personnel,
    required this.mostAssigned,
  ...
  Widget build(BuildContext context) {
    ...
    final stats = [
      (
        ...
        'Cobertura',
        total == 0 ? '0 %' : '${((active / total) * 100).round()} %',
        'Grados activos',
      ),
      (Icons.groups_outlined, 'Funcionarios', '$personnel', 'Asignados'),
      (
        Icons.trending_up_rounded,
        'Mayor personal',
        mostAssigned,
        '$mostCount funcionarios',
      ),
      (
                  ...
                  child: Row(
                    children: [
                      Container(
                        ...
                        child: Icon(s.$1, color: AdmTokens.primary, size: 20),
                      ),
